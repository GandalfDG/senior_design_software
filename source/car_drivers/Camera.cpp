...
void Camera::calibrate(void) {
	...
	calibration.rising_threshold = (calibration.max + calibration.min)
			/ 2+ THRESHOLD_WIDTH;
	calibration.falling_threshold = (calibration.max + calibration.min)
			/ 2- THRESHOLD_WIDTH;

}

void Camera::find_edges(uint16_t* camline) {
//	camera_data.right_edge_outer = find_edge_between(0, camera_data.prev_center + FUZZY_WIDTH, camline, FALLING, false);
	camera_data.right_edge_inner = find_edge_between(0, NUM_PIXELS / 2, camline, FALLING, true);
//	camera_data.left_edge_outer = find_edge_between
...
uint8_t Camera::find_edge_between(uint8_t lower_bound, uint8_t upper_bound,
	...
	if (!reverse) {
		for (int i = lower_bound >= 0 ? lower_bound : 0;
				i < (upper_bound <= NUM_PIXELS ? upper_bound : NUM_PIXELS)
						&& edge == -1; i++) {
			if (pol == RISING) {
